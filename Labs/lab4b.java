...
class Main{
	...
	public static void main(String[] args){
		...
		while(t-->0){
			...
			arr = arrx;
			int bigdifx = 0;
			int smalldifx = Integer.MAX_VALUE;
			merge_sort(0, q+1);

			for(int i = 0; i<q+1; i++){
				if (arr[i]-arr[i+1]==0){continue;}
				numx++;
				if (-(arr[i]-arr[i+1])<smalldifx){smalldifx = -(arr[i]-arr[i+1]);}
				if (-(arr[i]-arr[i+1])>bigdifx){bigdifx = -(arr[i]-arr[i+1]);}
			}
			int numy=0;
			arr = arry;
			int bigdify = 0;
			int smalldify = Integer.MAX_VALUE;
			merge_sort(0, q+1);

			for(int i = 0; i<q+1; i++){
				if (arr[i]-arr[i+1]==0){continue;}
				numy++;
				if (-(arr[i]-arr[i+1])<smalldify){smalldify = -(arr[i]-arr[i+1]);}
				if (-(arr[i]-arr[i+1])>bigdify){bigdify = -(arr[i]-arr[i+1]);}
			}
			int total = numy*numx;
			int maxarea = bigdify*bigdifx;
			int minarea = smalldify*smalldifx;
			System.out.println(total+" "+minarea+" "+maxarea);
