...
class Main{
    public static void main(String[] args) throws IOException{
        ...
        int c1s = Reader.nextInt()-1;
        int c2d = Reader.nextInt()-1;
        Node[] f = prem(graph, c1s, c2d);
        int ans = 0;
        for(Node i: f){
            if(i!=null){ans+=i.key;}//System.out.println((i.path+1)+" "+(i.name+1)+ " " +i.key);
        }
        System.out.println(ans);

    }

    static Node[] prem(Graph graph, int c1, int c2){
        ...
        while (!queue.isEmpty()) {
            ...
            for (NodeWE u :graph.adj[p.name]){ 
                ...
                if (visitar[u.destination] == false){
                    ...
                    // }
                    //w(u,v)<key[v];
                    if (f[u.destination].key > u.weight){
                        int k = linearsearch(e, f[u.destination]);
                        f[u.destination].key = u.weight;
        ...
        for(NodeWE u: graph.adj[c1]){
            if(u.destination==c2){f[c2].key = u.weight;break;}
        }
        return f;
    }
